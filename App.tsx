...
// (הקוד שמתחת ל-refreshFromDB עודכן)
  ...
  // מנגנון רענון אוטומטי (Real-time polling) כל 10 שניות לנציגים ומנהלים
  useEffect(() => {
    refreshFromDB();
  ...
  // פונקציית שליחת הודעה מנציג למנהל
  ...
  // ... שאר הפונקציות (handleLogin, handleLogout וכו') נשארות זהות

  // הזרקת הפונקציות והמידע ל-Components
  // (בתוך ה-Return, נעדכן את ה-props של RepPortal ו-MessagesManager)
      ...
      <div className="flex min-h-screen bg-[#F9FAFB] overflow-x-hidden font-sans relative">
        ...
        <main className={`flex-1 transition-all duration-300 ${showSidebar ? 'md:pr-72' : ''}`}>
          <div className="max-w-[1600px] mx-auto min-h-screen relative">
            {/* ... שאר ה-Routes ... */}
            {currentPage === 'rep_portal' ? (
              <RepPortal 
                rep={allRepresentatives.find(r => r.id === user?.id) || (user as any)} 
                patrols={patrols} 
                allReps={allRepresentatives} 
                donations={allDonations} 
                addDonation={addDonation} 
                paths={allPaths} 
                callLists={allCallLists} 
                updateDonorStatus={(did, s) => setAllDonors(prev => prev.map(d => d.id === did ? {...d, assignmentStatus: s} : d))} 
                systemMessages={systemMessages} 
                sendRepMessage={sendRepMessage} // חיבור הפונקציה החדשה
                donors={allDonors} 
              ...
              <MessagesManager 
                reps={filteredReps} 
                sendSystemMessage={(msg) => setSystemMessages(prev => [{...msg, id: Math.random().toString(36).substr(2,9), timestamp: new Date().toISOString()}, ...prev])} 
                incomingMessages={repToAdminMessages} 
                setIncomingMessages={setRepToAdminMessages} 
              />
            ) : (
              /* שאר הקוד המקורי שלך ללא שינוי */
              currentPage === 'registration' ? <Registration onRegister={(c) => setAllCustomers(prev => [c, ...prev])} onBack={() => setCurrentPage('login')} /> :
              currentPage === 'dev_handoff' ? <DevHandoff /> :
              user ? (
